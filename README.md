...
使用开源数据预训练权重微调时改用 `--pretrained`：

```bash
.venv/bin/python train.py \
  --config config/train.local.json \
  --pretrained checkpoints/run_xxx/best_model.pth
```

该模式优先加载 checkpoint 的 `params_ema`，只初始化在线模型权重；optimizer、EMA、global
step、最佳指标和学习率计划都会从头创建。`--pretrained` 与 `--resume` 互斥，也可以在配置的
`training.pretrained` 中设置权重路径。微调建议将 `max_lr` 调低到 `1e-5`～`2e-5`。
...
运行前确认 `dataset.manifest_path` 和 `training.pretrained` 与本机实际路径一致。默认配置使用
`256×256` LR ROI、batch size 4、30,000 steps，并将学习率从 `2e-5` 余弦衰减到 `1e-7`。
